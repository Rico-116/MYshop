...
func SendLoginCode(req models.SendLoginCodeRequest) error {
	if req.Email == "" {
		logger.Log.Warn("发送验证码失败：邮箱不能为空")
		return errors.New("邮箱不能为空")
	}

	if !isValidEmail(req.Email) {
		logger.Log.Warn("发送验证码失败：邮箱格式不正确",
			zap.String("email", req.Email),
		)
		return errors.New("邮箱格式不正确")
	}
	...
	if err != nil {
		logger.Log.Error("发送验证码失败：查询邮箱失败",
			zap.String("email", req.Email),
			zap.Error(err),
		)
		return errors.New("查询邮箱失败")
...
func SendResetPasswordCode(req models.SendResetPasswordRequest) error {
	if req.Email == "" {
		logger.Log.Warn("发送找回验证码失败：邮箱不能为空")
		return errors.New("邮箱不能为空")
	}

	if !isValidEmail(req.Email) {
		logger.Log.Warn("发送找回验证码失败：邮箱格式不正确",
			zap.String("email", req.Email),
		)
		return errors.New("邮箱格式不正确")
	}
	...
	user, err := dao.GetByEmail(req.Email)
	if err != nil {
		logger.Log.Error("发送找回验证码失败：查询邮箱失败",
			zap.String("email", req.Email),
			zap.Error(err),
		)
		return errors.New("查询邮箱失败")
	...
	if user == nil {
		logger.Log.Warn("发送找回验证码失败：该邮箱未注册",
			zap.String("email", req.Email),
		)
		return errors.New("该邮箱未注册")
	}
	...
	cooldownKey := fmt.Sprintf("reset_cooldown:%s", req.Email)

	cooldownExists, err := util.RDB.Exists(ctx, cooldownKey).Result()
	if err != nil {
		logger.Log.Error("发送找回验证码失败：查询发送冷却失败",
			zap.String("email", req.Email),
			zap.String("cooldown_key", cooldownKey),
			zap.Error(err),
		)
	...
	err = util.SendResetPasswordEmail(req.Email, code)
	if err != nil {
		logger.Log.Error("发送找回验证码失败：邮件发送失败",
			zap.String("email", req.Email),
			zap.Error(err),
		)
		return errors.New("邮件发送失败")
	...
	err = util.RDB.Set(ctx, codeKey, code, 5*time.Minute).Err()
	if err != nil {
		logger.Log.Error("发送验证码失败：验证码保存失败",
			zap.String("email", req.Email),
			zap.String("code_key", codeKey),
			zap.Error(err),
		)
	...
	err = util.RDB.Set(ctx, cooldownKey, 1, 60*time.Second).Err()
	if err != nil {
		logger.Log.Error("发送验证码失败：发送冷却保存失败",
			zap.String("email", req.Email),
			zap.String("cooldown_key", cooldownKey),
			zap.Error(err),
		)
...
func ResetPassword(req models.ResetPasswordRequest) error {
	if req.Email == "" {
		logger.Log.Warn("找回密码失败：邮箱不能为空")
		return errors.New("邮箱不能为空")
	}
	if req.Code == "" {
		logger.Log.Warn("找回密码失败：验证码不能为空", zap.String("email", req.Email))
		return errors.New("验证码不能为空")
	}
	if req.NewPassword == "" {
		logger.Log.Warn("找回密码失败，新密码不能为空", zap.String("email", req.Email))
	}
	if req.ConfirmPassword == "" {
		logger.Log.Warn("找回密码失败：确认密码不能为空",
			zap.String("email", req.Email),
		)
		return errors.New("确认密码不能为空")
	}
	...
	if req.NewPassword != req.ConfirmPassword {
		logger.Log.Warn("找回密码失败：两次输入的密码不一致",
			zap.String("email", req.Email),
		)
	...
	if !isValidEmail(req.Email) {
		logger.Log.Warn("找回密码失败：邮箱格式不正确",
			zap.String("email", req.Email),
		)
		return errors.New("邮箱格式不正确")
	}
	if len(req.NewPassword) < 6 {
		logger.Log.Warn("找回密码失败：新密码长度不足",
			zap.String("email", req.Email),
		)
		return errors.New("新密码不能少于6位")
	}

	user, err := dao.GetByEmail(req.Email)
	if err != nil {
		logger.Log.Error("找回密码失败：查询用户失败",
			zap.String("email", req.Email),
			zap.Error(err),
		)
	...
	codeKey := fmt.Sprintf("reset_code:%s", req.Email)
	cooldownKey := fmt.Sprintf("reset_cooldown:%s", req.Email)
	cacheCode, err := util.RDB.Get(ctx, codeKey).Result()
	if err != nil { //等于redis.Nil代表不存在,其他错误说明redis坏了，这个后期再决定用不用改
		logger.Log.Error("充值密码失败：验证码过期或者不存在", zap.String("email", req.Email))
		return errors.New("验证码过期或者不存在")
	}
	if cacheCode != req.Code {
		logger.Log.Warn("重置密码失败：验证码错误", zap.String("email", req.Email))
		return errors.New("验证码错误")
	}
	enPassword, err := util.HashPassword(req.NewPassword)
	if err != nil {
		logger.Log.Error("重置密码失败：密码加密失败", zap.String("email", req.Email), zap.Error(err))
		return errors.New("密码处理失败")
	}
	err = dao.UpdatePassword(req.Email, enPassword)
	if err != nil {
		logger.Log.Error("重置密码失败：更新密码失败", zap.String("email", req.Email), zap.Error(err))
		return errors.New("更新密码失败，请联系管理员")
	}
	if err = util.RDB.Del(ctx, codeKey).Err(); err != nil {
		logger.Log.Warn("重置密码成功，但删除验证码失败",
			zap.String("email", req.Email),
			zap.String("code_key", codeKey),
			zap.Error(err),
		)
	}
	if err = util.RDB.Del(ctx, cooldownKey).Err(); err != nil {
		if err = util.RDB.Del(ctx, codeKey).Err(); err != nil {
			logger.Log.Warn("重置密码成功，但删除验证码失败",
				zap.String("email", req.Email),
				zap.String("code_key", codeKey),
	...
	logger.Log.Warn("重置密码成功，但删除冷却key失败",
		zap.String("email", req.Email),
		zap.String("cooldown_key", cooldownKey),
		zap.Error(err),
	)
